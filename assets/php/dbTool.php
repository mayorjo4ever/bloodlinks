 ...
 @session_start(); set_time_limit(0); 
	...
	class DbTool{
			...
			try{
				...
				#$this->code = $this->conn->errorCode();7
				$this->getConn();	
				}
				catch(PDOException $er){					
					##  $this->code = $_SESSION['error_code'] = $er->getCode(); 
					##  $this->message = $_SESSION['error_msg'] = $er->getMessage(); 
					throw new Exception(" Error Code  ".$er->getCode().", Error Message: ".$er->getMessage());
				}	
			}			
			/// 
    ...
    //  prepare and execute it
		 ...
		 try{
			...
			if(empty($where)){
				...
				return $output;
					...
					//$this->message = $_SESSION['error_msg'] = $er->getMessage(); 
					// throw new Exception(" Error Code  ".$this->code.", Error Message: ".$this->message);
					throw new Exception(" Error Code  ".$er->getCode(), $er->getMessage());
		 }
		...
		#			
		  ...
		  $stm = $conn->prepare($str);
		   ...
		   // $this->status = true; 
		  ## deprecated in v8.2 status, message and code 
			...
			return $res = $stm->fetchAll();
					...
					//$this->code = $_SESSION['error_code'] = $er->getCode(); 
					//$this->message = $_SESSION['error_msg'] = $er->getMessage(); 
					throw new Exception(" Error Code  ".$er->getCode()." ,  message: ".$er->getMessage());
			 }
		  }
		  #
		  /****************************************************************************/
	...
	// make crypting
		...
		function decodeId($id){
		 ...
		 public function clean($text,$type='string'){
			 if($type=='string') return filter_var($text,FILTER_SANITIZE); 			
			 else return filter_var($text,FILTER_SANITIZE_EMAIL);
		} **/
		...
		function product_desc($id){
			$dbm = new DbTool(); 					 
			$id = $dbm->clean($id);
			$prod_info = explode('|',$id); ## brand / categ / type
			#####
			$info_01 = $dbm->getFields($dbm->select('product_brands',array('sn'=>$prod_info[0],'status'=>'active')),array('name','sn'));				
			$info_02 = $dbm->getFields($dbm->select('product_brands_categs',array('brand_id'=>$prod_info[0],'sn'=>$prod_info[1],'status'=>'active')),array('name','brand_id','sn'));
			$info_03 = $dbm->getFields($dbm->select('product_brands_categs_types',array('brand_id'=>$prod_info[0],'categ_id'=>$prod_info[1],'sn'=>$prod_info[2],'status'=>'active')),array('name','brand_categ','sn'));
			return $info_01['name'][0]." / ".
				   $info_02['name'][0]." / ".
				   $info_03['name'][0]."  "; 
		}
		function product_info($item_id){
			$dbm = new DbTool(); 					 
			$id = $dbm->clean($id);
			$fields = array('brand_id','categ_id','categ_type_id','name','barcode','purchase_price','sn',
						'selling_price','qty','date_sold','rec_finalized','ref_id','total_price','remains');    
			$item_info = $dbm->getFields($dbm->select('stock_items',
						array('sn'=>$item_id,'status'=>'active')),$fields);
			if(!is_null($item_info)) return   $dbm->resort($item_info); 
			else return $item_info; 
		}
		
		#############################################
		/*******************************************/
		function analyse_staff_salary_pay($user_id){  $dbm = new DbTool(); 
			$basic_salary = 0; $total_bonus = 0; $total_deduct = 0; $gross_pay = 0; 
			 
			$basic_salary_exists = $dbm->getFields($dbm->select('basic_salary',array('user_id'=>$user_id,'status'=>'active')),array('sn','user_id','amount'));
			  if(!is_null($basic_salary_exists)){ 
				 $basic_salary_exists = $dbm->resort($basic_salary_exists);
				 $basic_salary =  $basic_salary_exists['amount'];
			  }
			  ######################################
			  $allowance_exists = $dbm->getFields($dbm->select('staff_allowance',array('user_id'=>$user_id,'status'=>'active')),array('sn','user_id','amount','ref_id'));
			  if(!is_null($allowance_exists)) $total_bonus = array_sum($allowance_exists['amount']);
			  ######################################
			  $deduction_exists = $dbm->getFields($dbm->select('staff_deductions',array('user_id'=>$user_id,'status'=>'active')),array('sn','user_id','amount','ref_id'));
			  if(!is_null($deduction_exists)) $total_deduct = array_sum($deduction_exists['amount']);
			 ###################
			 $gross_pay  = $basic_salary + $total_bonus - $total_deduct; 
			 /*****/
			 return array('basic_salary'=>$basic_salary,'total_bonus'=>$total_bonus,'total_deduct'=>$total_deduct,'gross_pay'=>$gross_pay);
		 }
		 #############################################
		/*******************************************/
