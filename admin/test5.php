<?php 
		...
		// $all_specimen = $mydbm->runBaseQuery("SELECT DISTINCT rs.ticket_no, rs.bill_type_id, rs.date_c as date1,sp.date_perform FROM `customer_specimen_result` rs, customer_specimen sp where rs.status='active' and sp.status='active' and rs.bill_type_id=sp.bill_type_id and rs.ticket_no=sp.ticket_no LIMIT 5500,3000"); 
		
		$all_specimen = $mydbm->runBaseQuery("SELECT DISTINCT rs.ticket_no, rs.bill_type_id, rs.date_c as date1,sp.date_perform FROM `customer_specimen_result` rs, customer_specimen sp where rs.status='active' and sp.status='active' and rs.bill_type_id=sp.bill_type_id and rs.ticket_no=sp.ticket_no LIMIT 21470,20 "); 
		 
		
		// $tsum = 0;
		 if(!empty($all_specimen)){ $n = 0;
			foreach($all_specimen as $k=>$v){
				 //$mydbm->runBaseQuery("update customer_specimen set date_perform = '".$all_specimen[$k]['date1']."' where ticket_no = '".$all_specimen[$k]['ticket_no']."' and  bill_type_id = '".$all_specimen[$k]['bill_type_id']."'   and status ='active'");
				 ?><tr> 
					<td title='sn'> <?php echo $n+1; ?></td>
					<td title='ticket_no'> <?php echo $all_specimen[$k]['ticket_no']; ?></td>
					<td title='bill id'> <?php echo $all_specimen[$k]['bill_type_id']; ?></td>
					<td title='date1'> <?php echo $all_specimen[$k]['date1']; ?></td>
					<td title='comment'> <?php echo $all_specimen[$k]['date_perform']; ?></td>
		 ...
		 print "</table>";
		/**
		if(!empty($all_specimen)) foreach($all_specimen as $k=>$v){
			$tcost = get_ticket_sum_price($all_specimen[$k]['ticket_no']);
			  echo "\tID : ".$all_specimen[$k]['ticket_no']."\t price : ".$tcost." "; 
			 /// echo $dbm->updateTb('customer_tickets',array('total_cost'=>$tcost),array('ticket_no'=>$all_specimen[$k]['ticket_no'])); 
			  $tsum += $tcost;
			echo "<br/>";
		}**/
		//echo "<h3>\t\t$tsum</h3> \n\n\n";
		...
		/**
		$all_specimen = $mydbm->runBaseQuery("select * from customer_specimen where date_c  BETWEEN '2022-02-01' and  '2022-02-28' group by ticket_no"); 
		// $tsum = 0;
		if(!empty($all_specimen)) foreach($all_specimen as $k=>$v){
			$tcost = get_ticket_sum_price($all_specimen[$k]['ticket_no']);
			  echo "\tID : ".$all_specimen[$k]['ticket_no']."\t price : ".$tcost." "; 
