<?php 
	...
	if(isset($_POST['relogUser'])){
			...
			$password = $dbm->clean(strip_tags($_POST['password']));
	...
	if(isset($_POST['gettime'])){
			...
			echo  "<span class='text-info font-18'>".$d."</span>".",&nbsp;<span class='text-warning font-18'>".$t."</span>";
	}
	/****************************************************************************************/
	...
	if(isset($_POST['make_pharm_payment'])){
		$dbm = new DbTool(); # pharm_products
		$due = $dbm->clean($_POST['due']);
		$amount_paid = $dbm->clean($_POST['amount_paid']);
		$patient_ref = $dbm->clean($_POST['patient_ref']); ## hosp_no | host 
		   ...
		   <table class="table table-responsive table-hover"> 
				<tbody>
					<tr>
						...
						<td > <b> <?php echo $result_01['fullname'][$n];  ?> </b> &nbsp;<small> [ host ]</small> </td>
						<td class="bold font-33" rowspan="<?php echo (count($mysib['ref_no'])+1);?>"> <label class="badge badge-outline-dark font-18"> <?php echo $result_01['hosp_no'][$n];  ?> </label> </td>												
						<td class="text-capitalize">  
							<a class="btn btn-dark btn-sm bold text-white" data-text="<?php echo $id."|host|".$result_01['fullname'][$n]; ?>" title="Take Vital Science" onclick="set_vs_info($(this).attr('data-text'))" data-toggle="modal" data-target="#vitalScienceModal"> Vit. Sci <span class="fa fa-thermometer"></span></a> 
							<a class="btn btn-inverse-dark btn-sm bold" href="<?php echo "reg_slip.php?n=$myname&tp=$mytype&tp2=$mytype2&mln=$mymilno&hn=$myhsp&db=$dob&dtc=$mydate_c";?>" target="_blank" title="Print Registration Slip "> Reg. Slip &nbsp; <span class="fa fa-vcard-o "></span> </a> 
							<a class="btn btn-inverse-dark btn-sm bold" href="<?php echo "treatment_slip.php?n=$myname&tp=$mytype&tp2=$mytype2&mln=$mymilno&hn=$myhsp&db=$dob&dtc=$mydate_c";?>" target="_blank" title="Print Treatment Slip "> Tmt. Slip &nbsp; <span class="fa fa-stethoscope"></span></a> 
							<a onclick="load_all_bill_type($('#allBillType')),manage_receipt_view($(this).attr('data-text'))" data-toggle="modal" data-target="#createPatientBill" data-backdrop="static" data-keyboard="false" class="btn btn-inverse-dark btn-sm bold"   title="Create Payment Receipt " href="#" data-text="<?php echo $data_text; ?>"> Pay&nbsp; <span class="fa fa-money"></span></a> 
					...
					<?php 
						...
						?> 
					<tr> 
						 <td> <b><?php echo $mysib['fullname'][$m]; ?> </b>  &nbsp;  <small>[ <?php echo $mysib['type'][$m]; ?> ] </small> </td> 
						
						 <td>		
							<a class="btn btn-sm btn-dark bold text-white" data-text="<?php echo $sid."|".$mysib['type'][$m]."|".$mysib['fullname'][$m]; ?>" onclick="set_vs_info($(this).attr('data-text'))"  data-toggle="modal" data-target="#vitalScienceModal" title="Take Vital Science ">Vit. Sci <span class="fa fa-thermometer"></span></a> 
							<a class="btn btn-inverse-dark btn-sm" href="<?php echo "reg_slip.php?n=$myname&tp=$mytype&mln=$mymilno&hn=$myhsp&db=$dob&dtc=$mydate_c";?>" target="_blank" title="Print Registration Slip ">Reg. Slip &nbsp; <span class="fa fa-vcard-o "></span> </a> 
							<a class="btn  btn-inverse-dark btn-sm" href="<?php echo "treatment_slip.php?n=$myname&tp=$mytype&tp2=$mytype2&mln=$mymilno&hn=$myhsp&db=$dob&dtc=$mydate_c";?>" target="_blank" title="Print Treatment Slip">  Tmt. Slip &nbsp; <span class="fa fa-stethoscope"></span> </a> 
							<a onclick="load_all_bill_type($('#allBillType')),manage_receipt_view($(this).attr('data-text'))" data-toggle="modal" data-target="#createPatientBill" data-backdrop="static" data-keyboard="false" class="btn  btn-inverse-dark btn-sm" href="#" data-text="<?php echo $data_text; ?>" title="Create Payment Receipt"> Pay&nbsp; <span class="fa fa-money"></span> </a> 
							<!-- <br/>
							<a class="btn btn-info btn-sm" href="<?php echo "treatment_slip.php?n=$myname&tp=$mytype&mln=$mymilno&hn=$myhsp&db=$dob&dtc=$mydate_c";?>" target="_blank"> receipts </a> 
							 -->
						 </td> 
	...
	*/
	...
	if(isset($_POST['savePatientVitalScience'])){  
		...
		#####################
		$data = array('ref_no'=>$ref_no,'type'=>$type ,'fullname'=>$fullname,'temp'=>$temp,
	...
	<?php 
		...
		if(isset($_POST['getPatient'])){ 
			$conn = new mysqli('localhost', 'root', 'mayoskele', 'hrmpl'); 
			#$conn = new mysqli('localhost', 'root', '', 'hrmpl'); 
		...
		if($reqType == "default") { 
			$sql = $conn->query("SELECT * FROM patients order by date_c DESC LIMIT $start, $limit "); 
			$found = $conn->query("SELECT * FROM patients order by date_c DESC "); 
		}
		...
		if ($sql->num_rows > 0) {
			...
			while($data = $sql->fetch_array()) { $n++;
				...
				$addNewSib = "add_sibling_interface.php?refn=".base64_encode($data['hosp_no'])."&nm=".base64_encode($data['fullname'])."&mode=".base64_encode('new');
				$reg_slip = "reg_slip.php?n=".base64_encode($data['fullname'])."&tp=".base64_encode($data['category']." [ host ]")."&hn=".base64_encode($data['hosp_no'])."&db=".base64_encode($data['dob'])."&dtc=".base64_encode($data['date_c']);
						...
						<div class="card">							
							<div class="card-body">
								...
								<div class="col-md-10 col-sm-8" style="float:left;">
									...
									',&nbsp;&nbsp;'.$data['lga'].',&nbsp;&nbsp;'.$data['state'].'&nbsp;state, &nbsp;  <i class="fa fa-calendar"></i> '.$data['dob'].'&nbsp;&nbsp; <a href='.$new_med_record.' target="_blank" title="View Medical Reports"><i class="fa fa-medkit font-22"></i></a>&nbsp;									
									<br/><b>Next of Kin: </b>'.$data['nokname'].'&nbsp; (<b>'.$data['nokrelationship'].'</b>)&nbsp; <span class="fa fa-phone"></span>&nbsp;'.$data['nokphone'].'
									'.
									' </h4><hr/> <span class="h5 bold"> Siblings: <label class="badge badge-success">'.$totsib.'</label> &nbsp; <a target="_blank" href='.$addNewSib.' title="add more sibling " class="badge badge-primary text-white bold"><i class="fa fa-user-plus"></i></a> '.$all_siblings.' </span>'.
									'<p class="text-muted"> <i> created by : '.$data['createdby'].', &nbsp; on '. $data['date_c'].'   &nbsp;  : '.date('h:s A',$data['time_c']).' </i></p>
								</div> <!-- col-md-10 --> 
	...
	if(isset($_POST['savePatientMedicalReport'])){  
		 $ref_no = mysql_real_escape_string($_POST['ref_no']); 
		 $mytype = mysql_real_escape_string($_POST['mytype']); 
		 $date_rec = mysql_real_escape_string($_POST['date_rec']); 		
		 ...
		 $serial =  mysql_real_escape_string($_POST['serial']); 
			...
			# $output = " $mode -- $ref_no + $mytype +  $date_rec + $report_no + $report_type + $contents"; 
		 ### 
		 $dbm = new DbTool(); 		 
		 ########
		 $patient_info = $dbm->getFields($dbm->select('patients',array('hosp_no'=>$ref_no)),array('category','sn')); 		  
		 $date_info = explode('-',$date_rec); ## Y-m-d   0-1-2
		 ##### 
		 if($mode=="new"){
			  $data = array('rec_by'=>$_SESSION['admUser'],'ref_no'=>$ref_no,'type'=>$mytype,'category'=>$patient_info['category'][0],'time_vs'=>strtotime($date_rec),
		'content'=>$contents,'report_type'=>$report_type,'date_vs'=>$date_rec,
		'month_vs'=>$date_info[1],'year_vs'=>$date_info[0],'day_vs'=>$date_info[2],'week_vs'=>idate('W',strtotime($date_rec)),
		'date_c'=>date('Y-m-d'),'month_c'=>date('m'),'year_c'=>date('Y'),'week_c'=>idate('W',time()),'time_c'=>time()); 
		  
		 $dbm->insert('tickets_converse',$data);  		   
				echo json_encode(array('icon'=>'success','text'=>"Your comment has been successfully saved",'title'=>' New Report Saved'));
		 }
		 else if($mode=="update"){
			  
		 $data2 = array('upd_by'=>$_SESSION['admUser'],'content'=>$contents,'report_type'=>$report_type,'date_vs'=>$date_rec); 
		  
			 $dbm->updateTb('tickets_converse',$data2,array('sn'=>$serial));  		   
				echo json_encode(array('icon'=>'success','text'=>"Your comment has been successfully updated",'title'=>' Patient Report Updated'));
		 
		 } ## end if 
		}
	
		#### get_report_content
		if(isset($_POST['get_report_content'])){  
		 $serial = mysql_real_escape_string($_POST['serial']); 
		 $_SESSION['report_mode'] = "update";
		 $dbm = new DbTool(); 		 
		 $report = $dbm->resort($dbm->getFields($dbm->select('tickets_converse',array('sn'=>$serial)),array('sn','report_type','content','from_user_id','date_vs','month_vs','year_vs','week_vs','time_vs','time_c'))); 
		 echo json_encode(array('report_type'=>$report['report_type'],'content'=>stripslashes($report['content']),'date_vs'=>$report['date_vs']));
		 ########
		} 
