<?php 
	...
	#################
		...
		if(isset($_POST['save_new_customer_request'])){		$dbm = new DbTool(); 	 
			...
			/*********************************/
				...
				$data = array('surname'=>$surname,'othername'=>$othername,'fullname'=>$fullname,'age_text'=>$age, /**'age_type'=>$age_type,**/
					'sex'=>$sex,'phone'=>$phone,'doctor'=>$doctor,'hospital'=>$hospital,'consultant'=>$consultant,'clinical_details'=>$clinical_details);
			
				$updData = array('surname'=>$surname,'othername'=>$othername,'fullname'=>$fullname,'age_text'=>$age,/**'age_type'=>$age_type,**/
					'sex'=>$sex,'phone'=>$phone,'doctor'=>$doctor,'hospital'=>$hospital,'consultant'=>$consultant,'clinical_details'=>$clinical_details);
			switch($mode){
				case "new":{					
		...
		if(isset($_POST['add_this_specimen'])){		$dbm = new DbTool();
			...
			switch($_SESSION['ticket_mode']){
				...
				### save  ###
					if(is_null($exist)){
						$dbm->insert('customer_specimen',array_merge($data,array('date_upd'=>date('Y-m-d'),'time_upd'=>date('H:i:s',time()-3600))));  ## ,'date_upd'=>date('Y-m-d'),'time_upd'=>date('H:i:s',time()),
				...
				case "new":{
					...
					### save  ###
					if(is_null($exist)){
						$dbm->insert('customer_specimen',array_merge($data,array('date_c'=>date('Y-m-d'),'time_c'=>date('H:i:s',time()-3600))));  ## ,'upd_by'=>$_SESSION['admUser'],'date_upd'=>date('Y-m-d'),'time_upd'=>date('H:i:s',time()),
						echo json_encode(array('icon'=>'success','text'=>" Your Specimen : [ $specimen ], successfully added for processing",'title'=>'Specimen Added Successfully'));
					 }else {
						echo json_encode(array('icon'=>'error','text'=>"This Specimen : [ $specimen ], is already on the list, but you can specify another name for it. ",'title'=>'Duplicate'));
						}
											...
											<div id="<?php echo 'collapse'.$k; ?>" class="collapse" role="tabpanel" aria-labelledby="<?php echo 'heading'.$k; ?>" data-parent="<?php echo '#accordion'.$k; ?>">
											  <div class="card-body">
												...
												?>
													...
													&nbsp; &nbsp; 
													<button onclick="download_results($(this).attr('for'))" class="btn btn-primary btn-rounded bold  ladda-button" data-style="zoom-in" for="<?php echo base64_encode($rows[$k]['ticket_no']);?>" > Download Selected (Combined) </button>    
													<br/> 
												</div>
											</div>
										  </div>
	...
	if(isset($_POST['make_payment'])){ $dbm = new DbTool(); 
		...
		if($discount!="" && !is_numeric($discount )) {
			...
			'collected_by'=>$_SESSION['admUser'],'time_paid'=>date('H:i:s',time()-3600)); 
			/************************************/ 
			$fields = array('c_by','sn','ticket_no','fullname','total_cost','amount_paid','discount','date_c','time_c','date_fin','time_fin');
			$criterial = array('ticket_no'=>$ticket_no,'status'=>'active');
	...
	if(isset($_POST['reverse_payment'])){ $dbm = new DbTool(); 
		$ticket_no = base64_decode($dbm->clean($_POST['ticket_no']));
			...
			$dbm->updateTb('payment_log',array('status'=>'inactive','date_del'=>date('Y-m-d'), 
				'time_del'=>date('H:i:s',time()-3600),'del_by'=>$_SESSION['admUser']),
				 ...
				 <span class=" h5 text-success bold"> Existing Customers in the invoice for <?php echo $hosp_name; ?> </span>
					...
					if(!is_null($selected_tickets)) { $m=0;  
							...
							<tr>
								...
								<td colspan="2"> 
									 <button disabled type="button" onclick="remove_existn_customer($(this).attr('for'))"   for="<?php echo $hosp_id; ?>" class="btn btn-danger btn-rounded btn-lg remove-customer" name="remove-customer"> Remove <span class="exist_count"> 0 </span> Customer  &nbsp;   <i class="fa fa-times-circle font-20"></i> </button>
									  &nbsp; &nbsp; 
				...
				echo json_encode(array('icon'=>'error','text'=>'Amount Must Be Numeric','title'=>'Invalid Amount!'));  
				  ...
				  if(empty($invoice_exists)){
					...
					$result = $dbm->updateTb('hospital_invoice_report',array('amount_paid'=>$new_amount_paid,
						...
						'balance'=>$new_balance,'date_paid'=>$date_paid,'time_paid'=>$time_paid,),
