                ...
                <div class="card d-flex">
                  <div class="card-body" style="height:auto">
				   ...
				   <p style="height:140px;"> </p>
						...
						<?php 
							$invoice_no = filter_var(base64_decode($_REQUEST['a']),FILTER_SANITIZE_STRING);
							$paym_comp = filter_var(base64_decode($_REQUEST['b']),FILTER_SANITIZE_STRING);
						// echo "<span class='h3'>".$invoice_no."<br/>";
						// echo $paym_comp."</span>";
						$invoices = $dbm->getFields($dbm->select('hospital_invoice_report',array('invoice_no'=>$invoice_no,'status'=>'active')),array('sn','hosp_id','invoice_no','acct_id','total_cost','c_by','date_c','time_c'));
						...
						?>	
						 ...
						 <?php if(!is_null($tickets)){
							...
							<?php $p = 0;  $t_cost = 0; foreach($tickets['ticket_no'] as $ticket_no ){ $bill_name ="";
								...
								<tr class=""> 
									<td> <?php echo "&nbsp; ".($p+1); ?>  </td> 									
									<td> <?php echo $ticket_info['fullname'][0]; ?> </td> 
									<td> <?php echo ($tickets['alt_test_name'][$p]=="")?$bill_name:$tickets['alt_test_name'][$p]; ?></td> 
									<td> <?php echo  $dated; ?> </td> 									
									<td> <?php echo  "&#8358; ".number_format($ticket_info['total_cost'][0]); ?> </td> 									
								</tr> 
							
							<?php $p++;  } ## end foreach   ?>
								<tr class="bold">
									<td colspan="3" align="left"> <span class="h4 text-capitalize">Amount In Words:  <?php echo $func->num_to_word($t_cost)." naira only "; ?>  </span></td>
									<td align="left"> <span class="h4">TOTAL </span>  </td>
									<td ><span class="h4"> <?php echo "&#8358; ".number_format($t_cost); ?></span> </td>
								</tr>
							</tbody>
							</table>
