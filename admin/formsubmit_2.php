	...
	<?php 
				...
				case "new":{					
					$exist = $dbm->getFields($dbm->select('bill_types',$data),array('sn','name','dept_id'));
					 if(!is_null($exist)){
						...
						$newdata = array('price'=>$price,'estm_time'=>$estm_time,'estm_time_type'=>$estm_time_type,'specimen_sample'=>$specimen_sample,'c_by'=>$_SESSION['admUser'],'date_c'=>date('Y-m-d'),'time_c'=>date('H:i:s',time()));
		...
		## delete bill type  
			if(isset($_POST['del_bill_type'])){  	$dbm = new DbTool(); # #sleep(3);	
						$serial = $dbm->clean($_POST['serial']); 						
		...
		if(isset($_POST['save_new_customer_request'])){		$dbm = new DbTool(); 	 
			...
			/*********************************/
				...
				$data = array('surname'=>$surname,'othername'=>$othername,'fullname'=>$fullname,'age'=>$age,'age_type'=>$age_type,
					'sex'=>$sex,'phone'=>$phone,'doctor'=>$doctor,'hospital'=>$hospital,'consultant'=>$consultant,'clinical_details'=>$clinical_details);
			
				$updData = array('surname'=>$surname,'othername'=>$othername,'fullname'=>$fullname,'age'=>$age,'age_type'=>$age_type,
					'sex'=>$sex,'phone'=>$phone,'doctor'=>$doctor,'hospital'=>$hospital,'consultant'=>$consultant,'clinical_details'=>$clinical_details);
			switch($mode){
				case "new":{					
					$exist = $dbm->getFields($dbm->select('customer_tickets',array('c_by'=>$_SESSION['admUser'],'finalized'=>'no','status'=>'active')),array('sn','surname','othername'));
					 if(!is_null($exist)){
						$dbm->updateTb('customer_tickets',$data,array('c_by'=>$_SESSION['admUser'],'finalized'=>'no','status'=>'active'));
						$msg = 'New Customer Profile Successfully Updated ';
						echo json_encode(array('icon'=>'success','text'=>$msg,'title'=>' Profile Updated '));
					}
					else {
						$newdata = array('c_by'=>$_SESSION['admUser'],'date_c'=>date('Y-m-d'),'time_c'=>date('H:i:s',time())); 
						$dbm->insert('customer_tickets',array_merge($newdata,$data));
		...
		if(isset($_POST['add_this_specimen'])){		$dbm = new DbTool();
			...
			switch($_SESSION['ticket_mode']){
				...
				$data = array('bill_type_id'=>$serial,'specimen_sample'=>$specimen,'custom_ticket_id'=>$custom_ticket_id['sn'],'ticket_no'=>$_SESSION['ticket_no'],'finalized'=>'yes','upd_by'=>$_SESSION['admUser'],'status'=>'active');
				$exist = $dbm->getFields($dbm->select('customer_specimen',$data),array('sn','bill_type_id','specimen_sample'));					
				### save  ###
					if(is_null($exist)){
						$dbm->insert('customer_specimen',array_merge($data,array('date_upd'=>date('Y-m-d'),'time_upd'=>date('H:i:s',time()))));  ## ,'date_upd'=>date('Y-m-d'),'time_upd'=>date('H:i:s',time()),
				...
				case "new":{
					$custom_ticket_id = $dbm->getFields($dbm->select('customer_tickets',array('c_by'=>$_SESSION['admUser'],'finalized'=>'no','status'=>'active')),array('sn','surname','othername'));
					if(!is_null($custom_ticket_id)) $custom_ticket_id = $dbm->resort($custom_ticket_id);
					$criterial = array('custom_ticket_id'=>$custom_ticket_id['sn'],'c_by'=>$_SESSION['admUser'],'finalized'=>'no','status'=>'active'); 
					$data = array('bill_type_id'=>$serial,'specimen_sample'=>$specimen,'custom_ticket_id'=>$custom_ticket_id['sn'],'c_by'=>$_SESSION['admUser'],'finalized'=>'no','status'=>'active');
					$exist = $dbm->getFields($dbm->select('customer_specimen',$data),array('sn','bill_type_id','specimen_sample'));					
					### save  ###
					if(is_null($exist)){
						$dbm->insert('customer_specimen',array_merge($data,array('date_c'=>date('Y-m-d'),'time_c'=>date('H:i:s',time()))));  ## ,'upd_by'=>$_SESSION['admUser'],'date_upd'=>date('Y-m-d'),'time_upd'=>date('H:i:s',time()),
						echo json_encode(array('icon'=>'success','text'=>" Your Specimen : [ $specimen ], successfully added for processing",'title'=>'Specimen Added Successfully'));
					 }else {
						echo json_encode(array('icon'=>'error','text'=>"This Specimen : [ $specimen ], is already on the list, but you can specify another name for it. ",'title'=>'Duplicate'));
						}
		...
		if(isset($_POST['display_my_specimen'])){		$dbm = new DbTool(); 	 			
			...
			if(!is_null($exist)){ ?>
				<table class="table table-nogap jambo_table"><tbody>
				<tr class="table-primary bold text-capitalize font-12"><td> sn </td> <td> test type </td> <td> specimen type </td> <td> cost </td><td> manage </td> </tr>
				<?php $n = 0; $tcost = 0;  foreach($exist['bill_type_id'] as $serial){ 
				...
				?>
					<tr> 
						 ...
						 <td> <span class="badge badge-outline-info"> <?php echo $exist['specimen_sample'][$n]; ?> </span> </td>
						 <td> <?php echo "&#8358; ".number_format($bill_type['price'][0]);  ?> </td>						
						 <td>
							<div class="btn-group border border-white">  								
								<button class="btn btn-sm btn-rounded btn-danger" onclick="del_customer_specimen($(this).attr('data-text'))" data-text="<?php echo $exist['specimen_sample'][$n]."|".$bill_type['name'][0]."|".$exist['sn'][$n]; ?>"> <i class="fa fa-times"> </i> </button> 
						 </td>						
					</tr> 					 
					 <?php $n++; } # end foreach  ?> 
			...
			<?php  }  else { ?>
				<div class="col-md-6 offset-3" > <button class="btn btn-outline-info btn-block  btn-lg btn-rounded" type="button" data-toggle="modal" data-target="#specimen_bill_form" <i class="fa fa-plus"> </i> &nbsp; Add Specimen </div>
			<?php }
			/*********************************/
		}
		...
		if(isset($_POST['del_customer_specimen'])){		$dbm = new DbTool(); 	 
			...
			if(!is_null($exist)) {
				$dbm->updateTb("customer_specimen",array('status'=>'inactive', 'del_by'=>$_SESSION['admUser'],'date_del'=>date('Y-m-d'),'time_del'=>date('H:i:s',time())),array('sn'=>$serial));
		...
		if(isset($_POST['display_my_final_specimen'])){		$dbm = new DbTool(); 	 			
			switch($_SESSION['ticket_mode']){
				case "update":{  
				$custom_ticket_id = $dbm->getFields($dbm->select('customer_tickets',array('ticket_no'=>$_SESSION['ticket_no'],'finalized'=>'yes','process_completed'=>'no','status'=>'active')),array('sn','surname','othername','fullname','age','age_type','phone','email','sex','hospital','doctor','consultant','ticket_no','clinical_details'));
				if(!is_null($custom_ticket_id)) $custom_ticket_id = $dbm->resort($custom_ticket_id);
					$criterial = array('custom_ticket_id'=>$custom_ticket_id['sn'],'ticket_no'=>$_SESSION['ticket_no'],'finalized'=>'yes','process_completed'=>'no','status'=>'active'); 
					$exist = $dbm->getFields($dbm->select('customer_specimen',$criterial),array('sn','bill_type_id','specimen_sample'));
				...
				case "new":{  
					$custom_ticket_id = $dbm->getFields($dbm->select('customer_tickets',array('c_by'=>$_SESSION['admUser'],'finalized'=>'no','status'=>'active')),array('sn','surname','othername','fullname','age','age_type','phone','email','sex','hospital','doctor','consultant','ticket_no','clinical_details'));
					if(!is_null($custom_ticket_id)) $custom_ticket_id = $dbm->resort($custom_ticket_id);
					$criterial = array('custom_ticket_id'=>$custom_ticket_id['sn'],'c_by'=>$_SESSION['admUser'],'finalized'=>'no','status'=>'active'); 
					$exist = $dbm->getFields($dbm->select('customer_specimen',$criterial),array('sn','bill_type_id','specimen_sample'));
			...
			<table class=" table table-nogap" style="font-family:Comic Sans MS; border:none;"><tbody>
				<tr class="text-capitalize "> <td> <span class="bold"> Patient's Name : </span> &nbsp;&nbsp; <?php echo $custom_ticket_id['fullname']; ?>  </td> <td> <span class="bold">  Age : </span> &nbsp;&nbsp;<?php echo correct_age($custom_ticket_id['age'],$custom_ticket_id['age_type']); ?>  </td> <td> <span class="bold">  Sex : </span> &nbsp;&nbsp;<?php echo $custom_ticket_id['sex']; ?></td>  </tr>
				<tr class="text-capitalize "> <td colspan="3"> <span class="bold">   Clinical details : </span> &nbsp;&nbsp;<?php echo $custom_ticket_id['clinical_details']; ?>  </td>  </tr>
				<tr class="text-capitalize "> <td colspan="3">  <span class="bold">  Refered by : </span> &nbsp;&nbsp;<?php echo $custom_ticket_id['doctor']; ?>  </td> </tr>
				<tr class="text-capitalize "> <td colspan="3">  <span class="bold">  Referrer’s Address : </span>&nbsp;&nbsp; <?php echo $custom_ticket_id['hospital']; ?></td>  </tr>				
			</table>	 
			
				<table class="table table-nogap jambo_table"  style="font-family:Comic Sans MS;"><tbody>
				<tr class="table-primary bold text-capitalize font-12"><td> sn </td> <td> Test Type </td> <td> specimen type </td> <td> cost </td> </tr>
				<?php $n = 0; $tcost = 0;  foreach($exist['bill_type_id'] as $serial){ 
					$bill_type = $dbm->getFields($dbm->select('bill_types',array('sn'=>$serial,'status'=>'active')),array('sn','name','categ_id','dept_id','price','estm_time','estm_time_type'));
					$tcost += $bill_type['price'][0]; 
				?>
		...
		if(isset($_POST['create_ticket'])){		$dbm = new DbTool(); 	 
			...
			switch($_SESSION['ticket_mode']){
				...
				if(!is_null($custom_ticket_id)) $custom_ticket_id = $dbm->resort($custom_ticket_id);
					...
					if(!is_null($exist)){
					   /******* GET ALL COST PRICEE ******************************/
						$n = 0; $tcost = 0;  foreach($exist['bill_type_id'] as $serial){ 
						$bill_type = $dbm->getFields($dbm->select('bill_types',array('sn'=>$serial,'status'=>'active')),array('sn','name','categ_id','dept_id','price','estm_time','estm_time_type'));
						...
						echo json_encode(array('icon'=>'success','text'=>"TICKET NO : ".$_SESSION['ticket_no']." Has been successfully updated",'title'=>' Ticket Updates Successful '));
					} # end not null
				} break; 
				case "new":{
					$custom_ticket_id = $dbm->getFields($dbm->select('customer_tickets',array('c_by'=>$_SESSION['admUser'],'finalized'=>'no','status'=>'active')),array('sn','surname','othername','fullname','age','age_type','phone','email','sex','hospital','doctor','consultant','ticket_no'));
					if(!is_null($custom_ticket_id)) {
					$custom_ticket_id = $dbm->resort($custom_ticket_id);
					$criterial = array('custom_ticket_id'=>$custom_ticket_id['sn'],'c_by'=>$_SESSION['admUser'],'finalized'=>'no','status'=>'active'); 
					$exist = $dbm->getFields($dbm->select('customer_specimen',$criterial),array('sn','bill_type_id','specimen_sample'));
					if(!is_null($exist)){
						...
						# $total_cost;  $discount; $ticket_no;  $payment_completed; 
						$discount=0;  $finalized ='yes'; $amount_paid = 0; 
						...
						##### NOW UPDATE TICKET 
						 $dbm->updateTb('customer_tickets',$updData,array('c_by'=>$_SESSION['admUser'],'finalized'=>'no','status'=>'active'));
						 /******* UPDATE Specimen ******************************/
							$n = 0; foreach($exist['bill_type_id'] as $serial){ 
								$dbm->updateTb('customer_specimen',array('ticket_no'=>$ticket_no,'finalized'=>$finalized),$criterial); 
								$n++; } ## end foreach 
						/**********************************************************************/
						echo json_encode(array('icon'=>'success','text'=>"YOUR NEW TICKET ID IS : $ticket_no ",'title'=>' Ticket Created Successfully '));
					}
					else{
						echo json_encode(array('icon'=>'error','text'=>"No Specimen Found ",'title'=>'Invalid Request'));
					}
				} ## end not null customer 
				else {
					echo json_encode(array('icon'=>'error','text'=>"No Pending Customer Found ",'title'=>'Invalid Request'));
				} 
				} break; 
			}  # end switch 
		...
		if(isset($_POST['view_tickets'])){	$dbm = new DbTool(); $func = new functions(); 
			...
			if(!is_null($exist)){
				...
				$n = 0; ?> <div class="row">
				<?php foreach($exist['ticket_no'] as $ticket_no){ ?>
							  ...
							  <div class="d-flex justify-content-center">
								...
								<div class="d-flex flex-column ml-4">
								  <span class="d-flex flex-column">
									<p class="mb-0 bold"> <?php echo $exist['fullname'][$n]; ?> </p>
									<h4 class="font-weight-bold pointer">  <?php $link = base64_encode($ticket_no); $pc = base64_encode('no');  echo "<a href='process_ticket.php?r_val=$link&pc=$pc' target='_blank' class='unstyle'>". $ticket_no."</a>"; ?> </h4>
								  </span>
								  <small class="text-muted ">  <?php echo "<span class='mdi mdi-clock'></span>&nbsp; ".$func->format_date($exist['date_c'][$n])." -  ". $func->format_date($exist['time_c'][$n],'time') ; ?>  </small>
								</div>
							  </div>
							</div>
						  </div>
		...
		if(isset($_POST['view_tickets_to_modify'])){	$dbm = new DbTool(); $func = new functions(); 
			...
			$fields = array('c_by','sn','ticket_no','fullname','total_cost','amount_paid','discount','date_c','time_c','date_fin','time_fin');
			$tickets = $dbm->getFields($dbm->select_distinct('ticket_no','customer_specimen',array('to_modify'=>'yes','status'=>'active')),array('ticket_no'));
							...
							<div class="card-body">
							  <div class="d-flex justify-content-center">
								...
								<div class="d-flex flex-column ml-4">
								  ...
								  <small class="text-muted ">  <?php echo "<span class='mdi mdi-clock'></span>&nbsp; ".$func->format_date($exist['date_c'][0])." -  ". $func->format_date($exist['time_c'][0],'time') ; ?>  </small>
								</div>
							  </div>
							</div>
						  </div>
				...
				$dateto = $dbm->clean($_POST['dateto']);  # y-m-d
				 #### validate #### 
					if($datefrom > $dateto) {
						...
						if($datefrom == $dateto) $dates =  $mydbm->runBaseQuery("SELECT * FROM customer_tickets WHERE date_fin ='$datefrom' order by time_fin desc ");
						else $dates =  $mydbm->runBaseQuery("SELECT * FROM customer_tickets WHERE  date_fin >='$datefrom' and date_fin <='$dateto'  order by time_fin , date_fin desc ");
				...
				$dateto = $dbm->clean($_POST['dateto']);  # y-m-d
				 #### validate #### 
					if($datefrom > $dateto) {
						...
						# if($datefrom == $dateto) $dates  = mysql_query("SELECT * FROM payment_log WHERE date_paid ='$datefrom' AND status='active' order by time_paid desc ") or mysql_error();
						if($datefrom == $dateto) $dates = $mydbm->runBaseQuery("SELECT * FROM payment_log WHERE date_paid ='$datefrom' AND status='active' order by time_paid desc "); 
						else $dates = $mydbm->runBaseQuery("SELECT * FROM payment_log WHERE  date_paid >='$datefrom' and date_paid <='$dateto' AND status='active' order by time_paid , date_paid desc ");
				 ...
				 #### validate #### 
					if($datefrom > $dateto) {
						...
						if(!empty($dates)){ $rows = $dates;  ?>   
							...
							<?php  
							$sum_total = 0; $sum_paid = 0; $sum_discount = 0; $sum_bal = 0; 
							foreach($rows as $k=>$v){  $bal = $rows[$k]['total_cost'] - $rows[$k]['discount'] - $rows[$k]['amount_paid'];
								$sum_bal += $bal; $sum_total+= $rows[$k]['total_cost'];  $sum_paid+= $rows[$k]['amount_paid'];  $sum_discount += $rows[$k]['discount'];
							...
							?>
									<tr>  
									   ...
									   <td class=" text-capitalize"><?php echo "&#8358; ".number_format($rows[$k]['total_cost']); ?>  </td> 
									   <td class=" text-capitalize"><?php echo "&#8358; ".number_format($rows[$k]['amount_paid']) ?>  </td> 
									   <td class=" text-capitalize"><?php echo "&#8358; ".number_format($rows[$k]['discount']) ?>  </td>  
									   <td class=" text-capitalize"><?php echo "&#8358; ".number_format($bal) ?>  </td> 
								...
								<tr class="text-uppercase bold table-info h4 "> <td colspan="3" >total </td> <td>&#8358; &nbsp;<?php echo number_format($sum_total); ?> </td>  <td>&#8358; &nbsp;<?php echo number_format($sum_paid); ?> </td> <td> &#8358; &nbsp;<?php echo number_format($sum_discount); ?></td> <td> &#8358; &nbsp;<?php echo number_format($sum_bal); ?> </td> <td>&nbsp; </td> </tr>
		...
		function display_specimen_result_template($bill_type_id,$ticket_no){
			$dbm = new DbTool();
						...
						<p class="text-danger bold text-capitalize"> Use the checkbox to select the result type you want  </p>
							<table class="table table-nogap table-hover "> 
								<thead>
									...
									<tr> 
									<td colspan="3"> <div class="form-group"> <div class="col-sm-11"> <input type="text" name="test_comment" id="test_comment" class="form-control border border-primary bold font-16 input-sm" style="" placeholder="Test Comment ... " value="<?php echo $test_comment['comment']; ?>" /> </div></td> 
									<td  colspan="2"> <button onclick="save_specimen_result()" id="save_specimen_result_btn" data-text="<?php echo $ticket_no."|".$bill_type_id; ?>" class="btn btn-block btn-info btn-rounded btn-lg ladda-button" data-style="expand-right"> Save Report  &nbsp; <i class="fa fa-save"> </i> </button> </td>
									</tr> 
								</tbody>
		...
		if(isset($_POST['load_user_system_temp_values'])){
			...
			if(is_null($records)){ ## search template and fill there  | for textarea 
				 $criterial = array('bill_type_id'=>$info[1],'temp_type'=>'text_form','status'=>'active'); 
						...
						$exist = $dbm->getFields($dbm->select('specimen_result_template',$criterial),$fields);
					if(!is_null($exist))  echo $exist['raw_text_val'][0]; 						 
					}
			else  { echo $records['raw_text_result'][0]; }
		}
		/*****************************/
			...
			$n = 0;  
			 $dbm->deleteRow("customer_specimen_result",array('ticket_no'=>$ticket_no,'bill_type_id'=>$bill_type_id,'cur_state'=>'created','status'=>'active')); 
			   ...
			   foreach($temp_id as $cid){
				   ...
				   'date_c'=>date('Y-m-d'),'time_c'=>date('H:i:s',time())),$temp_info); 
				   $dbm->insert('customer_specimen_result',$data);
				 $n++;  
			   } # end foreach
				## update test result on specimen id  
				$dbm->updateTb("customer_specimen",array('comment'=>$test_comment),array('ticket_no'=>$ticket_no,'bill_type_id'=>$bill_type_id,'status'=>'active')); 
		...
		if(isset($_POST['save_specimen_text_result'])){ 	$dbm = new DbTool();		
			...
			$data = array('ticket_no'=>$ticket_no,'bill_type_id'=>$bill_type_id,'temp_type'=>'text_form',
				   ...
				   'date_c'=>date('Y-m-d'),'time_c'=>date('H:i:s',time())); 
			$updData = array('raw_text_result'=>$result,'upd_by'=>$_SESSION['admUser'],'date_upd'=>date('Y-m-d'),'time_upd'=>date('H:i:s',time())); 
			if(is_null($exist)){
				$dbm->insert('customer_specimen_result',$data);
				$dbm->updateTb("customer_specimen",array('comment'=>$test_comment),array('ticket_no'=>$ticket_no,'bill_type_id'=>$bill_type_id,'status'=>'active')); 
	...
	/***** view_ticket_analysis:'all',ticket_no:ticket_no,comp_status:comp_status  *******/ 
		if(isset($_POST['view_ticket_analysis'])){ 	$dbm = new DbTool();
			...
			$criterial = array('ticket_no'=>$ticket_no,'process_completed'=>$comp_status,'status'=>'active'); 		
			$exist = $dbm->getFields($dbm->select('customer_specimen',$criterial),$mydal->TableFields('customer_specimen'));
			$ticket_info = $dbm->resort($dbm->getFields($dbm->select('customer_tickets', array('ticket_no'=>$ticket_no)), $mydal->TableFields('customer_tickets')));
			if(!is_null($exist)){ ?>
			...
			<div class="row">
				<div class="col-md-6 float-left"> 
					<div class="form-group row selection">
							...
							<div class="col-sm-8">
								<div class="input-group">	 							
									<input style="font-size:14px; height:30px; " type="text" id="alt_test_name" name="alt_test_name" value="<?php echo $ticket_info['alt_test_name']; ?>" class="form-control border-primary" placeholder="Alt. Test Name"> 									
								</div>
							</div> <!-- ./ col-sm-8 -->
						  </div> <!-- ./ form-group -->
				</div> <!-- ./ col-md-6 -->
				...
				<div class="col-md-6 float-left"> 
					<div class="form-group row selection">
							...
							<div class="col-sm-8">
								<div class="input-group">									
									<input style="font-size:14px;  height:30px;" type="text" id="comment" name="comment" value="<?php echo $ticket_info['comment']; ?>" class="form-control border-primary" placeholder="Coment"> 									
								</div>
							</div> <!-- ./ col-sm-9 -->
						  </div> <!-- ./ form-group -->
				</div>
				...
				<tr class="table-primary bold text-capitalize font-12"><td class="serial"> sn </td>   <td> test type </td>  <td> result computed  </td> </tr>
				<?php $n = 0; $analysis = "";  foreach($exist['bill_type_id'] as $serial){ 
					...
					$result_saved = $dbm->getFields($dbm->select('customer_specimen_result',array('ticket_no'=>$ticket_no,'bill_type_id'=>$serial,'status'=>'active')),$mydal->TableFields('customer_specimen_result'));
					$analysis .= (is_null($result_saved))?"no|":"yes|"
				 ?> 
				<tr>  
				 <td class="serial"> <div class="icheck-square">  <input type="checkbox" name="specimen_results_check[]" value="<?php echo base64_encode($serial); ?>" class="checkbox specimen_results_check"  <?php echo (is_null($result_saved))?" disabled ":" checked "; ?> >  </div> </td> 
				 <td> <?php echo $bill_type['name'][0]; ?> </td>
				...
				<tr> <td colspan="3">  &nbsp; &nbsp; &nbsp; &nbsp;<button onclick="print_results($(this).attr('for'))" id="finalize_test_process" class="btn btn-success btn-rounded bold pull-right ladda-button" data-style="zoom-in" for="<?php echo base64_encode($ticket_no);?>" > Print Selected (Combined) </button>    
		...
		if(isset($_POST['finalize_test_process'])){  $dbm = new DbTool(); 	
			...
			$ticket_upd = array('comment'=>$comment,'alt_test_name'=>$alt_test_name,'process_completed'=>'yes','fin_by'=>$_SESSION['admUser'],'date_fin'=>date('Y-m-d'),'time_fin'=>date('H:i:s',time()));
		...
		if(isset($_POST['save_new_user'])){	// echo json_encode(array('icon'=>'success','text'=>'i can see you','title'=>'weldone'));
			...
			$psw = mysql_real_escape_string($_POST['psw']); 	$date_employ = $dbm->clean($_POST['date_employ']);
			...
			/***************************/
			 $data = array('surname'=>$surname,'firstname'=>$firstname,'midname'=>$othername,'fullname'=>$fullname,
				'gender'=>$sex,'dob'=>$dob,'address'=>$address,'phone'=>$phone, 'password'=>$psw,
				'enc_psw'=>md5($psw),'user_id'=>$user_id, 'c_by'=>$_SESSION['admUser'],'date_employ'=>$date_employ);
		...
		if(isset($_POST['load_page_groups'])){  		
					$dbm = new DbTool(); 	
					$types = $dbm->getFields($dbm->select('pagegroups',array('status'=>'active'),array('groupid'),'and','asc'),array('sn','groupname','groupid','icon'));					
				  ?> 
						<optgroup label="Page Group">
						<option value="">...</option>
						<?php	$n = 0; if(!is_null($types)) foreach ($types['groupid']  as $val){ 
								# $dtext = $val."|".$types['dept_id'][$n]."|".$types['categ_id'][$n]."|".$types['price'][$n];
								?>
									<option value="<?php echo $val; ?>"  <?php echo ($_SESSION['page_group']==$val)?"selected":"" ?>> <?php echo $types['groupname'][$n]." (".$val.")"; ?></option>							
						<?php $n ++; } ?>					 
						</optgroup>	
					<?php  
		}
	...
	function  get_new_ticket_id(){
		...
		$cur_year = date('y'); /**  abbrv 2 digit **/ $pay_type = 'labtest';
		$finalized ='yes';
		
		$allTransc = $dbm->getFields($dbm->select('customer_tickets',array('pay_type'=>$pay_type,'year'=>$cur_year,'finalized'=>$finalized)),array('sn','ticket_no'));
		
		$tot = count($allTransc['ticket_no']);
		
		$lastNo = $tot-1;
		
		$lastId = $allTransc['ticket_no'][$lastNo];  // $con->getFields($lastApp,array('applc_id'));
		...
		return trim("HRM/$cur_year/$newpad");  
	...
	if(isset($_POST['search_ticket_payment'])){  # search_ticket_payment:"new", ticket_no:value 
		$dbm = new dbTool(); $func = new functions();  $ticket_no = filter_var($_POST['ticket_no'],FILTER_SANITIZE_STRING);
			$fields = array('c_by','sn','ticket_no','fullname','total_cost','amount_paid','discount','date_c','time_c','date_fin','time_fin','payment_completed','payment_finalized','paym_date_fin','paym_time_fin','paym_fin_by');
			...
			if(is_null($exist)) { echo "<div class='alert alert-warning'> <i class='fa fa-warning'> </i> &nbsp;Ticket &nbsp; $ticket_no&nbsp; Not Found </div>"; }
					...
					$specimen = $dbm->getFields($dbm->select('customer_specimen',$criterial),array('sn','bill_type_id','specimen_sample'));
					$n = 0; $tcost = 0;  
					$my_specimen = ""; 
					foreach($specimen['bill_type_id'] as $serial){ 
					 $bill_type = $dbm->getFields($dbm->select('bill_types',array('sn'=>$serial,'status'=>'active')),array('sn','name','categ_id','dept_id','price','estm_time','estm_time_type'));
						$my_specimen.=" ".$bill_type['name'][0]."&nbsp;/ ".$specimen['specimen_sample'][$n]." :  &#8358; ".	number_format($bill_type['price'][0])." <br/> "; 
					}
			...
			<div class="row"> 
				...
				<div class="col-md-5 rounded float-left">
					...
					<label class="col-sm-8 col-form-label bold text-uppercase">  <?php echo "  &#8358; ".number_format($exist['total_cost']);?>  </label>
					</div> <div class="form-group row"> 
					<label class="col-sm-4 col-form-label"> Discount&nbsp; :  </label>
					<div class="col-sm-8">
						<div class="input-group">									
							<input style="font-size:14px; height:45px;" <?php echo ($exist['payment_completed']=="no")?"":" disabled "?>  type="text" onkeyup="calc_remains($('#discount'))" data-text="<?php echo $balance; ?>" id="discount" name="discount" value="0" class="form-control border-primary only-numeric" placeholder="0"> 							
						</div>
					</div> <!-- ./ col-sm-9 -->
					</div> <div class="form-group row"> 
					...
					<div class="col-sm-8">
						<div class="input-group">									
							<input style="font-size:14px; height:45px;"  <?php echo ($exist['payment_completed']=="no")?"":" disabled "?> type="text" id="amount_payable" name="amount_payable" value="<?php echo $balance; ?>" class="form-control border-primary only-numeric" placeholder="<?php echo $balance; ?>"> 							
						</div>
					</div> <!-- ./ col-sm-9 -->
					</div> <div class="form-group row"> 
					<label class="col-sm-4 col-form-label"> Payment Mode&nbsp; :  </label>
					<div class="col-sm-8">						 
                          <div class="form-group" > <label class="input-label"> &nbsp;<input tabindex="7" <?php echo ($exist['payment_completed']=="no")?"":" disabled "?>  type="radio" name="paymode" class="paymode radio"  value="cash" checked />
                          CASH </label>  </div>  
						  <div class="form-group"> <label class="input-label" > &nbsp;<input tabindex="8" <?php echo ($exist['payment_completed']=="no")?"":" disabled "?>  type="radio" name="paymode" class="paymode radio"  value="pos" />
                          POS </label>   </div> 
						  <div class="form-group">   <label class="input-label"> &nbsp;<input tabindex="9" <?php echo ($exist['payment_completed']=="no")?"":" disabled "?>  type="radio" name="paymode" class="paymode radio" value="transfer" />
                          TRANSFER </label>  </div>
					</div> <!-- ./ col-sm-8 -->
				</div> <div class="form-group row"> 
					...
					<div class="col-sm-8 float-left">
						<?php 
							if($exist['payment_completed']=='no'){ ?> 
								<button type="button"  for="<?php echo base64_encode($ticket_no); ?>" data-text="<?php echo base64_encode($balance); ?>" onclick="make_payment()" class="make_payment btn btn-info btn-block btn-rounded btn-lg ladda-button" data-style="zoom-in"> Pay </button>
								<?php } 
								else { ?>
									<button type="button" <?php echo ($exist['payment_finalized']=="no")?"":" disabled "?> for="<?php echo base64_encode($ticket_no); ?>" data-text="<?php echo base64_encode($balance); ?>" onclick="finalize_payment()" class="finalize_payment btn btn-success btn-block btn-rounded btn-lg ladda-button" data-style="zoom-in"> Finalize Payment </button>
								<?php }	
							?> 
				...
				<div class=" col-md-7 rounded float-left">
					...
					<label class="col-sm-8 col-form-label bold text-capitalize pull-right">  <?php echo $func->format_date($exist['date_c'])."&nbsp; / ".$func->format_date($exist['time_c'],'time'); ?>  </label>
					...
					<label class="col-sm-4 col-form-label">   &nbsp; :  </label>
					<div class="col-sm-8"> <button type="button" <?php echo ($exist['payment_finalized']=="no")?"":" disabled "?> onclick="reverse_payment()" for="<?php echo base64_encode($ticket_no); ?>" class="reverse_payment btn btn-danger btn-sm btn-rounded"> Reverse Payment &nbsp; <i class="fa fa-undo"> </i> </button> </div>
				</div> </div> </div> </div>
			...
			<div class="row">  
							...
							<?php # load all payments 
							
							$m = 0; $myPayments = $dbm->getFields($dbm->select('payment_log',array('ticket_no'=>$ticket_no,'status'=>'active')),array('sn','expc_pay','discount','amount_paid','date_paid','time_paid'));
								 if(!is_null($myPayments)){  ?>
									<div class="col-12 grid-margin">
									  <div class="card card-statistics">
										<div class="row"> 
								<?php
								if($exist['payment_finalized']=="yes"){ $url = "receipt.php?r_val=".base64_encode($ticket_no)."&prd=".base64_encode(time())."&tkm=".base64_encode('Complete Payment'); ?>
								 <div class="card-col col-xl-4 col-lg-4 col-md-4 col-6">
									<div class="card-body "> <a href="<?php echo $url; ?>" class="unstyle text-dark" target="_blank">
									  <div class="d-flex align-items-center justify-content-center flex-column flex-sm-row">
										<i class="fa fa-check-circle font-24 text-success mr-0 mr-sm-4 icon-lg"></i>
										<div class="wrapper text-center text-sm-left">
										<p class="card-text mb-0 font-13"><?php echo $func->format_date($exist['paym_date_fin'])." ".$func->format_date($exist['paym_time_fin'],'time'); ?> </p>
										  <div class="fluid-container">
											<h3 class="mb-0 font-weight-medium">&#8358; <?php echo number_format($exist['total_cost']); ?> <small><b>Original</b></small></h3>
										  </div>
										</div>
									  </div> </a>
									</div>
								...
								<?php } 
								 ...
								 foreach($myPayments['amount_paid'] as $amounts){ $url = "receipt.php?r_val=".base64_encode($ticket_no)."&prd=".base64_encode(time())."&tkm=".base64_encode('invoice')."&rcpid=".base64_encode($myPayments['sn'][$m]);
							?>	 <div class="card-col col-xl-4 col-lg-4 col-md-4 col-6">
									<div class="card-body"> <a href="<?php echo $url; ?>" class="unstyle text-dark" target="_blank">
									  <div class="d-flex align-items-center justify-content-center flex-column flex-sm-row">
										<i class="fa fa-calendar font-20 text-primary mr-0 mr-sm-4 icon-lg"></i>
										<div class="wrapper text-center text-sm-left">
										<p class="card-text mb-0 font-13"><?php echo $func->format_date($myPayments['date_paid'][$m])." ".$func->format_date($myPayments['time_paid'][$m],'time'); ?> </p>
										  <div class="fluid-container">
											<h3 class="mb-0 font-weight-medium">&#8358; <?php echo number_format($amounts); ?></h3>
										  </div>
										</div>
									  </div> </a>
									</div>
								...
								echo "<div class='col-md-12'><div class='alert alert-warning h3 '> no payment have been made  </div> </div>";
							}
								?>
	...
	if(isset($_POST['make_payment'])){ $dbm = new DbTool(); 
		...
		if($discount!="" && !is_numeric($discount )) {
			...
			'discount'=>$discount,'amount_paid'=>$amount_paid,'date_paid'=>date('Y-m-d'),
			'collected_by'=>$_SESSION['admUser'],'time_paid'=>date('H:i:s',time())); 
			/************************************/ 
			$fields = array('c_by','sn','ticket_no','fullname','total_cost','amount_paid','discount','date_c','time_c','date_fin','time_fin');
			$criterial = array('ticket_no'=>$ticket_no,'status'=>'active');
	...
	if(isset($_POST['reverse_payment'])){ $dbm = new DbTool(); 
		$ticket_no = base64_decode($dbm->clean($_POST['ticket_no']));
			...
			$criterial = array('ticket_no'=>$ticket_no,'status'=>'active');  
				$specimens = $dbm->getFields($dbm->select('customer_specimen',$criterial),array('sn','bill_type_id','specimen_sample'));
				if(!is_null($specimens)){
					/*******GET ALL COST PRICEE ******************************/
					$n = 0; $total_cost = 0;  foreach($specimens['bill_type_id'] as $serial){ 
					$bill_type = $dbm->getFields($dbm->select('bill_types',array('sn'=>$serial,'status'=>'active')),array('sn','name','categ_id','dept_id','price','estm_time','estm_time_type'));
					$total_cost += $bill_type['price'][0]; 
					$n++; } ## end foreach
				}	## end not null 	 			
			...
			## reset payment log 
			$dbm->updateTb('payment_log',array('status'=>'inactive','date_del'=>date('Y-m-d'), 
				'time_del'=>date('H:i:s',time()),'del_by'=>$_SESSION['admUser']),
	...
	if(isset($_POST['finalize_payment'])){ $dbm = new DbTool(); 
		$ticket_no = base64_decode($dbm->clean($_POST['ticket_no']));
			...
			$criterial = array('ticket_no'=>$ticket_no,'status'=>'active');  
			/**********************************************************************/
			$upd_data = array('payment_finalized'=>'yes','paym_fin_by'=>$_SESSION['admUser'],'paym_date_fin'=>date('Y-m-d'),'paym_time_fin'=>date('H:i:s',time())); 
			$dbm->updateTb('customer_tickets',$upd_data,$criterial);
			## reset payment log 
	...
	if(isset($_POST['auto_search_bill_for_ticket'])){ $dbm = new DbTool(); 
		...
		if(!empty($word)) { 
			$info = $dbm->getFields($dbm->regExpSearch('bill_types', array('name'=>$word),array('name'), " DESC ",'10'),array('name','categ_id','sn','dept_id','specimen_sample'));
			$tot = count($info['name']);
			 if(!is_null($info)){
			   $l=0; $m=0;
	...
	if(isset($_POST['auto_search_ticket_for_update'])){ $dbm = new DbTool(); 
		$word = $dbm->clean($_POST["keyword"]); 
		if(!empty($word)) { 
	...
	if(isset($_POST['auto_search_ticket_for_payment'])){ $dbm = new DbTool(); 
		...
		if(isset($_POST['save_system_info'])){ $dbm = new DbTool();
			...
			if(!filter_var($sys_email,FILTER_VALIDATE_EMAIL)){
				...
				$upd_data = array('name'=>$sys_name,'shortcut'=>$sys_shortname,'theme'=>$sys_theme,'fa_icon'=>$sys_icon,'email'=>$sys_email,'phone'=>$sys_phone,'address'=>$sys_address); 
	...
	####
		if(isset($_POST['get_staff_salary_structure'])){ $dbm = new DbTool(); $admin = new User("users");	
			  $user_id = base64_decode(filter_var($_POST['user_id'],FILTER_SANITIZE_STRING));
		...
		*****/
		...
		if(isset($_POST['save_my_salary_details'])){ $dbm = new DbTool(); 
			...
			if(!is_null($bonus_ref)){ $m = 0; 
				...
				foreach($bonus_ref as $ref_id){
					$amount = (double)filter_var(str_replace(',','',$bonus[$m]),FILTER_SANITIZE_STRING);
			...
			if(!is_null($debit_ref)){ $m = 0; 
				...
				foreach($debit_ref as $ref_id){
					$amount = (double)filter_var(str_replace(',','',$debits[$m]),FILTER_SANITIZE_STRING);
		...
		if(isset($_POST['save_staff_basic_salary'])){	$dbm = new DbTool(); 
			$staff_id = filter_var($_POST['staff_id'],FILTER_SANITIZE_STRING);
			$amount =  (double)filter_var(str_replace(',','',$_POST['basic_salary']),FILTER_SANITIZE_STRING);
		...
		if(isset($_POST['pay_the_staff_salary'])){ $dbm = new DbTool(); 
			 # print_r($_POST);
			 $staff = $_POST['staff'];  // staff id list [array] 
			 $paym_method = filter_var($_POST['paym_method'],FILTER_SANITIZE_STRING);  // bank / cash / transfer 
			 $paym_status = filter_var($_POST['paym_status'],FILTER_SANITIZE_STRING);  // paid			 
			 $param = filter_var($_POST['param'],FILTER_SANITIZE_STRING);    
			 $calendar = explode("|",$param); // year | month	
					...
					if(is_null($exists)){
						...
						if(!is_null($my_allowances)){ $i=0; 
							foreach($my_allowances['ref_id'] as $ref_id){
								...
								'ref_id'=>$ref_id,'amount'=>$my_allowances['amount'][$i],'date_paid'=>date('Y-m-d'),'time_paid'=>date('H:i:s',time()),'c_by'=>$_SESSION['admUser']));
								$i++;
							}
						}
						/***************************************/
						...
						if(!is_null($my_deductions)){ $i=0; 
							foreach($my_deductions['ref_id'] as $ref_id){
								...
								'ref_id'=>$ref_id,'amount'=>$my_deductions['amount'][$i],'date_paid'=>date('Y-m-d'),'time_paid'=>date('H:i:s',time()),'c_by'=>$_SESSION['admUser']));
								$i++;
							}
						}
						/***************************************/
	...
	/*******************************************************/
	 ...
	 /**********************************************************************/
	 	...
	 	if(isset($_POST['saveNewProduct'])){  		
				...
				$no_of_pack = mysql_real_escape_string($_POST['no_of_pack']);	
				$qty_per_pack = mysql_real_escape_string($_POST['qty_per_pack']);	
	...
	####
	 	if(isset($_POST['update_new_import_stock'])){  		
				...
				$update_serial = mysql_real_escape_string($_POST['update_serial']);	// new or update
				$dbm = new DbTool(); 
				$products = $dbm->getFields($dbm->select("pharm_products",
				array('sn'=>$update_serial,'status'=>'active')),
				array('sn','name','remains','description','code','barcode','exp_date','mfc_date',
					'cost_price','selling_price','qty','vendor_id','date_suplied','qty_per_pack','no_of_pack','rem_qty_per_pack','rem_no_of_pack'));	
	...
	// functions
		...
		if(isset($_POST['save_this_hospital'])){	$dbm = new DbTool(); 
			$hosp_name = filter_var($_POST['hosp_name'],FILTER_SANITIZE_STRING);
	...
	/******************************************************************/
		...
		/*****************************************/
		 ...
		 if(isset($_POST['add_customer_invoice'])){   $dbm = new DbTool(); 					 
			...
			if(is_null($exist)){ # save this to invoice 
				$data = array('hosp_id'=>$hosp_id,'ticket_no'=>$ticket_no,'c_by'=>$_SESSION['admUser'],'date_c'=>date('Y-m-d'),'time_c'=>date('H:i:s',time()));
		...
		if(isset($_POST['create_bank_account'])){
			$dbm = new DbTool(); 
			$staff_id = filter_var($_POST['staff_id'],FILTER_SANITIZE_STRING);
			$bank_info = explode("|",base64_decode(filter_var($_POST['bank_list'],FILTER_SANITIZE_STRING)));
			$bank_id = $bank_info[0];
	...
	############################################################################
		if(isset($_POST['create_invoice_memo'])){ $dbm = new DbTool(); 
			$hosp_id = filter_var($_POST['hosp_id'],FILTER_SANITIZE_STRING);
			$acct_id = filter_var($_POST['acct_id'],FILTER_SANITIZE_STRING);
			/***********************/
			...
			if(is_null($invoices)){
				...
				foreach($invoices['ticket_no'] as $ticket_no){
					$ticket_info = $dbm->resort($dbm->getFields($dbm->select('customer_tickets',array('ticket_no'=>$ticket_no,'status'=>'active')),array('total_cost')));
					$t_cost += $ticket_info['total_cost'];
				}
				$text = "Invoice Created Successfully for ".$hosp_info['name'][0]." @ ".$hosp_info['address'][0];
				$title = "Invoice #$invoice_no Was Created For ".$hosp_info['name'][0]. " Successfully with cost of N".number_format($t_cost);
				### invoice table 
				 $dbm->updateTb('hospital_invoice',array('inv_prepared'=>'yes','invoice_no'=>$invoice_no,'prep_by'=>$_SESSION['admUser'],'date_prep'=>date('Y-m-d'),'time_prep'=>date('H:i:s',time())),array('hosp_id'=>$hosp_id,'inv_prepared'=>'no','status'=>'active'));
				### invoice_report table 
				 $dbm->insert('hospital_invoice_report',array('hosp_id'=>$hosp_id,'invoice_no'=>$invoice_no,'acct_id'=>$acct_id,'total_cost'=>$t_cost,'c_by'=>$_SESSION['admUser'],'date_c'=>date('Y-m-d'),'time_c'=>date('H:i:s',time())));
